...
#include "zwealthsilocontroldialog.h"
#include "ui_zwealthsilocontroldialog.h"
...
std::list<std::string> ZWEALTHSILOControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZWEALTHSILOControlDialog::listMints;

ZWEALTHSILOControlDialog::ZWEALTHSILOControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZWEALTHSILOControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZWEALTHSILOControlDialog::~ZWEALTHSILOControlDialog()
{
    delete ui;
}

void ZWEALTHSILOControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZWEALTHSILOControlDialog::updateList()
{
...
void ZWEALTHSILOControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZWEALTHSILOControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZWEALTHSILO_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZWEALTHSILOControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZWEALTHSILOControlDialog::GetSelectedMints()
{
...
void ZWEALTHSILOControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
