...
void PrivacyDialog::on_pushButtonZWEALTHSILOControl_clicked()
{
    ZWEALTHSILOControlDialog* zWEALTHControl = new ZWEALTHSILOControlDialog(this);
    zWEALTHControl->setModel(walletModel);
    zWEALTHControl->exec();
}

void PrivacyDialog::setZWEALTHSILOControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzWEALTH()
{
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " WEALTHSILO </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZWEALTHSILOControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZWEALTHSILOControlDialog::GetSelectedMints();
    }
    ...
    // Clear zwealthsilo selector in case it was used
    ZWEALTHSILOControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " WEALTHSILO, ";
