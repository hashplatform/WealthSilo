...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the WEALTHSILO and zWEALTH money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 45595, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in WEALTHSILO/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in WEALTHSILO/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, wealthsilo, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in WEALTHSILO/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzwealthsilo=<n>", strprintf(_("Enable automatic wallet backups triggered after each zWEALTH minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizewealthsiloamount=<n>     " + strprintf(_("Keep N WEALTHSILO anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The WEALTHSILO Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZWEALTHSILOMinted();
                        RecalculateZWEALTHSILOSpent();
                    }
                    RecalculateWEALTHSILOSupply(1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZWEALTHSILOBackups = GetBoolArg("-backupzwealthsilo", true);
        pwalletMain->setZWEALTHSILOAutoBackups(fEnableZWEALTHSILOBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    if (nAnonymizeWEALTHSILOAmount < 2) nAnonymizeWEALTHSILOAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1WEALTHSILO+1000 == (.1WEALTHSILO+100)*10
       10WEALTHSILO+10000 == (1WEALTHSILO+1000)*10
    */
